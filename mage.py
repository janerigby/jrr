...
def redo_open_S99_spectrum(rootname, denorm=True, altfile=None) : 
    ...
    else :        S99file = S99path + rootname + '-sb99-fit.txt'
    ...
    if denorm : 
        ...
        # line below fails for chuck spectrum.  Write workaround
        if rootname == "chuck" :  orig_sp =  read_chuck_UVspec()
            ...
            sp['wave'] = sp['rest_wave'] * (1.0 +  orig_zz_syst)  # put the observed wavelength back in
...
def read_chuck_UVspec(mage_mode="released") :
    ...
    df = pandas.read_table(litfile, delim_whitespace=True, comment="#")
