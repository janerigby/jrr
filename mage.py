...
def wrap_open_spectrum(label, mage_mode) :  # Convenience wrapper, open one MagE spectrum in one line.  label is one short_name
    (spec_path, line_path) = getpath(mage_mode)
...
def open_S99_spectrum(rootname, zz) :
    ...
    *** NEED TO UDATE BC THIS IS NOW FLAM, not FNU ****
    '''
    mage_mode = "released"
    (spec_path, line_path) = getpath(mage_mode)
...
def redo_open_S99_spectrum(rootname) :  #, zz) :
    ''' Reads a best-fit Starburst99 spectrum generated by John Chisholm.  It's in the rest-frame already
    Outputs: a pandas data frame that contains some of the columns that open_spectrum makes.
    *** NEED TO UDATE BC THIS IS NOW FLAM, not FNU. TESTING NOW ****
    *** This flam is norm to ~1.  fnu is weird norm.  Once working, remove above.
    '''
    ...
    norm1 = orig_sp['rest_flam'][orig_sp['rest_wave'].between(*norm_region)].median()
    norm2 = sp['rest_flam_data_norm'][sp['rest_wave'].between(*norm_region)].median()    
    ...
    # STOPPED HERE.
    # TO DO  TEST ALL OF THIS *****  rest_fnu_data looks a lot like fnu.
    # There is a small redshift offset between sp and orig_sp.  Plot, send to JC, ask for help.
...
default_stacked_mage = "magestack_bystars_standard_spectrum.txt"
def open_stacked_spectrum(mage_mode, alt_infile=False, colfnu='X_avg', colfnuu='X_sigma') :
    ''' Open a stacked MagE spectrum. By default, grabs default_stacked_mage
    Rewritten 8/2016 to make stacked spectrum look like a normal spectrum, so can re-use tools.
    Optional arguments are:
     - alt_infile: alternative stacked spectrum to grab (just the file, path from indir),
    ...
    else : infile = indir + default_stacked_mage
    ...
    sp.rename(columns = {'restwave' : 'wave'}, inplace=True)
    sp['fnu'] = sp[colfnu]
    sp['fnu_u'] = sp[colfnuu]    
