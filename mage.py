...
def get_boxcar4autocont(sp) :
    target = 350. # Angstroms  trial and error, no real physics here.  This is ~990 pixels for the MagE spectra (not stacked)  observed-frame
    boxcar = np.int(util.round_up_to_odd(target / sp.disp.median()))
    return(boxcar) # in pixels
...
def wrap_open_spectrum(label, mage_mode) :  # Convenience wrapper, open one MagE spectrum in one line.  label is one short_name
    ...
    zz = specs.z_stars.values[0]
    infile = specs.filename[0]
    (sp, resoln, dresoln) = open_spectrum(infile, zz, mage_mode)
    linelist = get_linelist_name(infile, line_path)
    (LL, z_systemic) =  get_linelist(linelist)
    boxcar = get_boxcar4autocont(sp)
    auto_fit_cont(sp, LL, zz, boxcar=boxcar)
    return(sp, resoln, dresoln, LL, z_systemic)
