...
def wrap_load_1spectrum(Z, age, logU, style) :  #uses parameters rather than filenames
    should_look_like = 'BPASSv2.1_imf135_100/spectra.z020.dat.gz'  # TESTING NOW
    if 'binary' in style   : fileroot = 'spectra-bin'
    elif 'single' in style : fileroot = 'spectra'
    else : raise Exception("Misparsed style")
    filename = default_filenames()[0] + fileroot + '.z' + Z + '.dat.gz'
    print "DEBUGGING, filename", filename, "should look like", should_look_like
