...
def get_names_primary_mirror_segments():
    mirrornames = [y + str(x) for x in range(1,7) for y in ('A', 'B', 'C')]
    mirrornames.sort()
    return(mirrornames)
...
def get_nirspec_good_fixedslit_names():
    return('S200A1', 'S200A2', 'S400A1', 'S1600A1')
...
def get_NIRSpec_allgratings_resolution_uniformillum(wave, whichsetup='F100LPG140H', whichslit='S200A1'):
    ...
    infile = 'nirspec_measured_R_uniformillum_fromconvolution_allFS.pkl'
    polyfit = joblib.load(infile)
    label = whichsetup + '_' + whichslit
    return(polyfit[label](wave))
...
def get_coords_dayofyear_from_jwst_hdu(hdu, verbose=False, output_datetime=False):
    ...
    if output_datetime:  return(RA, DEC, dayofyear, datetime)
    else:                return(RA, DEC, dayofyear)
...
def get_background_for_jwstfile(jwstfile, bkg_file='/tmp/background.txt', plot_background=False, plot_bathtub=False, wave=2.0, showsubbkgs=False, debug=False, bathtub_file='background_versus_day.txt') :
# wrapper around the wrapper, that opens the JWST file and imports the resulting background
    hdu = fits.open(jwstfile)
    get_background_for_hdu(hdu, bkg_file=bkg_file, wave=wave, plot_background=plot_background, plot_bathtub=plot_bathtub, \
            debug=debug, bathtub_file=bathtub_file)
    # open and process the bkg file into dataframe
    bkg_df = open_background_file(bkg_file)
    return(bkg_df)

def get_background_for_hdu(hdu, bkg_file='/tmp/background.txt', wave=2.0, debug=False, plot_background=False, plot_bathtub=False, showsubbkgs=False, bathtub_file='background_versus_day.txt') : # assume jwst datafile is open
...
#    #wave = getwave_for_filter(thisfilt)  # wave is only used for the bathtub plot; I can't figure out how to get it from header
    ...
    jbt.get_background(RA, DEC, wave, thisday=dayofyear, thresh=1.1, plot_background=plot_background, plot_bathtub=plot_bathtub, \
            background_file=bkg_file, write_bathtub=True, showsubbkgs=showsubbkgs, bathtub_file=bathtub_file)
    return(0)
...
def merge_JBTbackground_w_spectrum(df_sp, df_bkg, prefix='jbt_', colwav_sp = 'wave', colwav_bkg='wave') :
    # Interpolate a JBT predicted background into an actual spectrum file (dataframe)
    for thiskey in df_bkg.keys():
        df_sp[prefix + thiskey] = rebin_spec_new(df_bkg[colwav_bkg], df_bkg[thiskey], df_sp[colwav_sp])
    df_sp.drop(prefix + colwav_bkg, axis=1, inplace=True)
    return(0) # acts on df_sp
...
def open_MIRI_JRsummaryfile(infile):
    ...
    calib_df       = df.loc[df.symbol == 'MIRI_imager_6616sinu']  # Fits to the calibration programs PID 6616 and 8303
    return(df, black_df, red_df, df_glowsticks, df_corona, raw_df, calib_df)
...
def jwst_load_pitchroll_database(telemfile=None):
    ...
    if telemfile == None:
        ...
        telemfile = '/Users/jrrigby1/Python/JWST_scienceops/RollPitch/' + 'jwst_sunpitch_sunroll_JR_062025.csv'
...
def jwst_lookup_1_pitchandroll(timetofind, df_pitchroll, timecol='decimalyr') :   # time is in decimal years
    ...
    #print("DEBUG", timetofind, subset.head())
...
def find_science_extensions_nirspecFS(infile):
    ...
    for thisslit in fixed_slit_names:
        out_dict[thisslit] = []
        sci_extensions = find_science_extensions(infile, kind='SCI')
        for ii in sci_extensions:
            sltname = gethead(infile, 'SLTNAME', extension=ii)
            if sltname == thisslit:
